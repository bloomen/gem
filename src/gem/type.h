...
public:
    ...
    template <typename... Args>
    static type<Managed>
    make_with(std::shared_ptr<std::shared_mutex> mutex, Args&&... args)
    {
        type<Managed> obj;
        obj.managed_ = std::make_shared<Managed>(std::forward<Args>(args)...);
    ...
    static type<Managed>
    from(std::shared_ptr<Managed> managed)
    {
        type<Managed> obj;
    ...
    static type<Managed>
    from_with(std::shared_ptr<std::shared_mutex> mutex,
              std::shared_ptr<Managed> managed)
    {
        type<Managed> obj;
        obj.managed_ = std::move(managed);
        obj.mutex_ = std::move(mutex);
    ...
    static type<Managed>
    null_with(std::shared_ptr<std::shared_mutex> mutex)
    {
        type<Managed> obj;
        obj.mutex_ = std::move(mutex);
        return obj;
    ...
    type<Managed>
    copy(const bool inherit_mutex = false) const
    {
        type<Managed> obj;
        if (managed_)
        {
    ...
    type<Managed>
    safe_copy(const bool inherit_mutex = false) const
    {
        std::shared_lock lock{*mutex_};
