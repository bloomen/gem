...
{
public:
    explicit data_error(const std::string& message)
...
public:
    ...
    virtual void on_data_changed(
        const std::shared_ptr<gem::ds::data>& value) = 0;
};
...
public:
    ...
    void
    add_observer(std::weak_ptr<gem::ds::observer> observer)
    {
    ...
    void
    remove_observer(const std::weak_ptr<gem::ds::observer>& observer)
    {
...
public:
    ...
    explicit value(std::string name)
        : gem::ds::data{gem::ds::data_type<value_type>::type_index,
                        std::move(name)}
    {
    }
    ...
    value(std::string name, T&& value)
        : gem::ds::data{gem::ds::data_type<ValueType>::type_index,
                        std::move(name)}
...
template <typename ValueType>
std::shared_ptr<gem::ds::value<std::decay_t<ValueType>>>
make_value(std::string name)
{
    return std::make_shared<gem::ds::value<std::decay_t<ValueType>>>(
        std::move(name));
}

template <typename ValueType>
std::shared_ptr<gem::ds::value<std::decay_t<ValueType>>>
make_value(std::string name, ValueType&& value)
{
    return std::make_shared<gem::ds::value<std::decay_t<ValueType>>>(
        std::move(name), std::forward<ValueType>(value));
}

template <typename ValueType>
std::shared_ptr<gem::ds::value<ValueType>>
cast_value(std::shared_ptr<data> value)
{
    if (data_type<ValueType>::type_index != value->type())
    {
        throw gem::ds::data_error{
            "Bad cast: ValueType does not fit value's data type"};
    }
...
template <typename ValueType>
std::string
to_string(const std::shared_ptr<gem::ds::value<ValueType>>& value,
          const char delim = '|')
{
    if (data_type<ValueType>::type_index != value->type())
    {
    ...
    auto str =
        std::to_string(value->type()) + std::string(1, delim) + value->name();
    if (value->get())
    {
...
template <typename ValueType>
std::shared_ptr<gem::ds::value<ValueType>>
from_string(const std::string& data, const char delim = '|')
{
    ...
    {
        throw gem::ds::data_error{
            "Expect either two or three elements in string"};
    }
    auto value = tokens.size() == 2
        ? gem::ds::make_value<ValueType>(tokens[1])
        : gem::ds::make_value(
              tokens[1], gem::ds::data_type<ValueType>::from_string(tokens[2]));
    if (std::stoi(tokens[0]) != value->type())
    {
        throw gem::ds::data_error{
            "ValueType does not match data type found in string"};
    }
    return value;
}
...
#define GEMDS_VALUE(name, value) auto name = gem::ds::make_value(#name, value)
